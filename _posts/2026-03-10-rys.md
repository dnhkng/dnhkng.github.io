...
![Open LLM Leaderboard](/assets/img/rys/leaderboard.jpg)
...
![By Original: Marxav, Vectorization: Mrmw - Own work based on: Full GPT architecture.png:, CC0, https://commons.wikimedia.org/w/index.php?curid=146645810](/assets/img/rys/Full_GPT_architecture.svg.png)
...
![Qwen2-72B](/assets/img/rys/math_eq_delta.png)
...
![Combined analysis](/assets/img/rys/combined.png)
...
![Maths analysis](/assets/img/rys/math.png)
...
![Maths analysis](/assets/img/rys/skyline_math.png)


*Until we look at the EQ scores:*

![EQ analysis](/assets/img/rys/eq.png)
...
![EQ analysis](/assets/img/rys/skyline_eq.png)
...
![Single Repeats](/assets/img/rys/heatmap_math_repeat_columns_horizontal_clipped_r1-4_compact.png)
...
![GLM-4.7 analysis](/assets/img/rys/GLM-4.7-heatmap_math_diff_clipped.png)
...
![GLM-4.7 analysis](/assets/img/rys/heatmap_math_diff_clipped_subsection_i27-42_j27-48.png)
...
![Llama-3-70B](/assets/img/rys/Llama-3-70B-Instruct_delta_clipped_side_by_side.png)

![Phi-3-medium](/assets/img/rys/Phi-3-medium-4k-instruct_delta_clipped_side_by_side.png)

![GPT-OSS-120B](/assets/img/rys/GPT-OSS-120B_delta_clipped_side_by_side.png)

![Qwen3-30B-A3B](/assets/img/rys/Qwen3-30B-A3B_delta_clipped_side_by_side.png)
...
A blog post can't be peer-reviewed, but it *can* be cited. Somewhat to my surprise, this one has been, mostly by people working on looped and recurrent Transformers who arrived at the same neighbourhood from a different direction.

### In the literature

In chronological order:

- **[CircuitProbe: Predicting Reasoning Circuits in Transformers via Stability Zone Detection](https://arxiv.org/abs/2604.00716)** — Rajkiran Panuganti, arXiv:2604.00716, April 2026.
  Predicts where the duplicable circuits are instead of brute-forcing the sweep the way I did, and reports close to the opposite of my scaling conclusion: gains below 3B, degradation at 7B and above. No idea if it works; not tested it. On the small models I'm not sure we disagree — they're *noisy*, and you can nearly always find a configuration that lifts *some* benchmark; what I never found down there was one that lifted them all. The top end is another matter. Their circuits are predicted rather than searched, so a null result at 7B+ may say more about the predictor than about duplication, and their sweep stops an order of magnitude short of where I was working. RYS-XLarge is a 78B model on HuggingFace, with benchmarks the leaderboard ran rather than me.

- **[Inference-Path Optimization via Circuit Duplication in Frozen Visual Transformers for Marine Species Classification](https://arxiv.org/abs/2604.03428)** — Thomas Manuel Rost, arXiv:2604.03428, April 2026.
  The first port of circuit duplication to computer vision: duplicating layers in a frozen DINOv3 stack for underwater species classification, with class-specific circuit selection reaching a macro F1 of 0.875 (octopus alone gained +12.1 points). Frozen weights, no gradient descent, better model. Exactly the trick, on a completely different modality — which is the strongest evidence yet that this is a property of Transformers, not of Qwen.

- **[The Topological Trouble With Transformers](https://arxiv.org/abs/2604.17121)** — Michael C. Mozer, Shoaib Ahmed Siddiqui, Rosanne Liu, arXiv:2604.17121, April 2026.
  A theoretical argument that purely feedforward stacks are fundamentally limited at dynamic state tracking, and that recurrence is the fix. RYS is cited among the handful of methods that add depth recurrence *purely at inference time*.

- **[LoopUS: Recasting Pretrained LLMs into Looped Latent Refinement Models](https://arxiv.org/abs/2605.11011)** — Taekhyun Park, Yongjae Lee, Dohee Kim, Hyerim Bae, arXiv:2605.11011, May 2026.
  Post-trains a pretrained model into an explicit encoder / looped-reasoning-block / decoder architecture. That decomposition is the neuroanatomy hypothesis of this post, made structural: *"Ng [18] and Upstage show that duplicating or stacking pretrained blocks can improve performance."*

- **[MACRO: Markov Chain Routing of Transformer Layers](https://arxiv.org/abs/2608.05872)** — Paweł Batorski, Abtin Pourhadi, Akylgali Aitaza, Przemysław Spurek, Paul Swoboda, arXiv:2608.05872, August 2026.
  Learns a context-dependent Markov policy over layer routes (skip, repeat, residual) instead of searching for a fixed one, reporting ~5% average gains and cutting search time from 14.8 hours to 1.6. My method is listed as the exhaustive-search baseline: *"Neuroanatomy exhaustively searches a limited route space."* Guilty as charged — that was two RTX 4090s and a lot of patience.

- **[Recirculation](https://arxiv.org/abs/2608.17981)** — Michael C. Mozer, Shoaib Ahmed Siddiqui, Danny Sawyer, Sunny Sanyal, Rosanne Liu, arXiv:2608.17981, August 2026.
  Mixes activations back into earlier layers rather than re-running them, for a 23% perplexity reduction and +21% on GSM8k for Gemma3. Interestingly, they report that *looping* (my approach) doesn't give robust benefits on the Gemma3 family — consistent with my own finding that every architecture has its own neuroanatomy, and that smaller, more entangled models don't have a cleanly separable reasoning cortex. Their framing of "model-design affordances" — finding out how a pretrained model *wants* to be modified, rather than imposing an architecture and paying for it with training — is, I think, exactly the right way to describe what the heatmaps are showing us.

- **[The Generalization Ridge: Information Flow in Natural Language Generation](https://arxiv.org/abs/2507.05387v6)** — Ruidi Chang, Chunyuan Deng, Hanjie Chen, arXiv:2507.05387 (v6), August 2026.
  Tracks how much predictive information each layer holds about the target as training goes on. Across GPT-2, Qwen and LLaMA it peaks in the intermediate layers — the "generalization ridge" — before falling off in the final layers, which they read as the model sliding from generalization into memorization. RYS arrived in the sixth revision, as *"additional intervention-based evidence that layers differ substantially in their functional contributions."* A nice complement from the measurement side: they find the most generalizable representations sitting in the middle of the stack, which is also where the heatmaps kept telling me to duplicate.

### In software

- **[x-transformers](https://github.com/lucidrains/x-transformers)** — Phil Wang (AKA lucidrains - the god of Open Source transformer coding).
  The BibTeX block below appears in the citation list of x-transformers, among the depth-recurrence work that fed into the library — sitting next to Zhu et al.'s looped language models and Mozer et al.'s recirculation, both of which it actually implements. There's no RYS feature in the code as far as I can tell, so this is credit as prior art rather than an implementation. Fine with me.
