---
title: "2x GH200 for LLM inference, Part 4: DeepSeek V4 Flash, DSpark, and SGLang vs vLLM at 1M context"
...
## The misleading 26.33 GiB failure
...
vLLM is tuned, the 1M context is validated, and it works. The one problem that did not go away is TileLang. The mHC kernels compile *during serving*, so on a 100-request run the median TTFT is fine (~0.28 s) **but p99 rises to ~11.5 s**: occasionally a live request stalls for ten-plus seconds while TileLang compiles a shape it had not warmed. Swapping the mHC kernels for a PyTorch/Triton fallback flattens p99 to ~2.6 s but costs about 65% of decode, so within vLLM the choice is a poor one: keep the throughput and accept the tail stalls, or remove the stalls and give back most of the decode speedup.

This will probably be solved upstream; it is fundamentally a warmup-coverage problem, and each release compiles more shapes before serving. For now the vLLM setup here is a working solution, tail spikes and all. But an occasional ten-second stall today, with the fix only somewhere on the roadmap, was reason enough to see what else would run on this box.

## SGLang, now that it runs on ARM64

Back when I wrote Parts 2 and 3, SGLang was not a candidate on this machine: it did not build cleanly on aarch64, so the series has been vLLM (and llama.cpp) by necessity, not by preference. That has changed. **SGLang now has real ARM64 support**, so it runs on this GH200 box. I brought it up natively: the upstream Dockerfile handles ARM now, skips the missing `sgl-flash-attn3` cubins, and JITs them at runtime instead. Both GPUs come up at compute capability 9.0. After a few fixes, SGLang beat the tuned vLLM config on every decode workload I tested.

### The silent shared-expert loader bug
...
Here is where the original local fix landed, against the same tuned vLLM numbers from above. The vLLM prefill column is no-spec, which favours vLLM slightly since the draft path adds prefill work:
...
The original patch remains the fastest code row, but the combined upstream native-FP8 build is the one I would deploy. It preserves the checkpoint's shared-expert precision, remains 11.9% faster than the separately tuned vLLM `k=6` code result, and does not depend on silently converting the FP8 shared expert to FP4. A paired five-story replay produced different text for all five prompts. Both profiles were coherent in this tiny sample; native FP8 reached a natural stop in four of five responses, versus three of five for fused FP4. That is not a quality benchmark, but it is enough to show that the precision change is behaviourally material. The speed spread is also too small to justify making fusion the default.
...
Both SGLang profiles completed all 100 requests without a live compile, JIT, OOM, or unexpected shared-expert event in the server log. The native-FP8 run is the apples-to-apples comparison with vLLM and had the better decode and end-to-end p99 in these particular runs. This is strong observed evidence, but still one warmed run per profile rather than a universal tail guarantee; fresh-cache repetitions, more prompt shapes, and a long-lived soak are still warranted.

### SGLang at the full 1M context

The second earlier caveat said vLLM alone owned 1M on this box. That is no longer true. The first SGLang validation did reach the limit, but only by forcing a fixed 1,024-token prefill chunk. That bounded the DSV4 indexer's temporary workspace, but it turned a full prefill into a 895.4-second exercise.

The underlying tradeoff is not really "speed profile versus capacity profile." Short prompts want the 12,288-token chunk because it avoids extra full-model continuation passes; extremely long prompts need progressively smaller chunks so the indexer workspace does not consume the memory reserved for weights and KV cache. I added a small adaptive policy to the scheduler. This is a local patch: stock SGLang has no such knob, so the environment variables below only exist on a build that carries it, and it is the piece of this setup I would most like to propose upstream. The policy:
...
Every row generated one additional token. The selected native-FP8 run therefore exercised **1,048,536 total tokens**, 40 below the configured model limit, and immediately afterwards served a fresh 8K request at 9,836 tok/s, so the server returns to normal speed right after a million-token prompt. The adaptive native profile is 60.0% lower-latency than fixed 1K and retains the measured short-context fast path, so SGLang's 1M mode is no longer merely a 15-minute capacity demonstration.
