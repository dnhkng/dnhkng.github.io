...
**And to be clear: none of this is a quantum computer.** There are no entangled states, no superposition of token sequences, no Grover's algorithm searching the multiverse for the winning trade. The rig is a classical pipeline driven by single-event quantum measurements. Each time a token is generated by the connected LLM, it was selected *based on whether single photons were detected on either the left or right path by a photomultiplier tube.* The branching happens because every measurement decoheres into its own world, not because anything is being *computed in parallel*.
...
You type a question. The server fires attenuated light at the beam splitter until it accumulates five bits, which it uses to index into the 20 answer faces on the die inside a Mattel Magic 8-Ball. The selected answer comes back as JSON. One question, five bits, twenty answers, twenty multiverse branches.

Wait, isn't 5 bits 32 possible outcomes? I thought too long and hard about this, and we could do something fancy, like reweighting the 20-sided die:
...
**1. Encode detector imbalance.** A measured imbalance is captured by:

$$\delta = |2p - 1|$$

so a perfect 50:50 split gives $\delta = 0$, and a 55:45 split gives $\delta = 0.1$.

**2. Let the bias compound across a raw address.** A token's CDF address is 32 bits wide. If each bit carries a multiplicative bias, the relative variance of hitting one specific address compounds across all 32 bits, one $(1 + \delta^2)$ factor per bit:

$$V(\delta) = (1 + \delta^2)^{32} - 1$$

The exponent is just the uint32 address width — nothing more exotic than that.

**3. Average over a token's slots.** A token allocated $K$ slots is the sum of $K$ roughly independent address-probabilities, so averaging pulls the standard deviation down by the usual $\sqrt{K}$:

$$\sigma_K \approx \sqrt{\frac{V(\delta)}{K}}$$

**4. Pick the safety margin.** We don't want detector noise to drag a token's effective probability down toward half its intended value. That "half intended" line sits a fractional distance $0.5$ below the mean, and we want a fat $5.5\,\sigma$ cushion before we reach it:
...
But sitting at the bench, watching the PMTs blink, watching the balance bleed out, I kept thinking about the actual physical chain on my desk. Photon path. PMT pulse. FPGA bit. Spreader. CDF lookup. Token. Tool call. HTTP request. Order book.
