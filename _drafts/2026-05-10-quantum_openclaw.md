...
**And to be clear: none of this is a quantum computer.** There are no entangled states, no superposition of token sequences, no Grover's algorithm searching the multiverse for the winning trade. The rig is a classical pipeline driven by single-event quantum measurements. Each time a token is generated by the connected LLM, it was selected *based on whether single photons were detected by either a left or right path to some photomultiplier tube.* The branching happens because every measurement decoheres into its own world, not because anything is being *computed in parallel*. 
...
Tier totals: 31.25% yes, 31.25% no, 37.5% maybe. But I opted for boring: we grab 5 bits, and if the number is over 20, we just resample. 
...
Using a 250k vocabulary and a Zipf slope around (s=1.8), which gives a low-entropy next-token distribution — top token about 53%, top 10 around 90% — we get:
...
The default $K$ should not be pulled from the air. We can derive it from the one
hardware defect we know we will never fully escape: the PMT pair imbalance.
...
**From one biased bit to one address.** A token's CDF address is 32 bits wide. If
each bit carries a multiplicative bias, the relative variance of hitting one
specific address compounds across all 32 bits, one $(1 + \delta^2)$ factor per bit:

$$V(\delta) = (1 + \delta^2)^{32} - 1$$

The exponent is just the uint32 address width — nothing more exotic than that.

**From one address to one token.** A token allocated $K$ slots is the sum of $K$
roughly-independent address-probabilities, so averaging pulls the standard
deviation down by the usual $\sqrt{K}$:

$$\sigma_K \approx \sqrt{\frac{V(\delta)}{K}}$$

**The safety margin.** We don't want detector noise to drag a token's effective
probability down toward half its intended value. That "half intended" line sits a
fractional distance $0.5$ below the mean, and we want a fat $5.5\,\sigma$ cushion
before we reach it:
...
and since $(5.5 / 0.5)^2 = 11^2 = 121$, the coefficient is not a magic number — it
is the margin-over-threshold ratio, squared:

$$K_{\text{bias}} = \left\lceil 121\left((1+\delta^2)^{32}-1\right) \right\rceil$$

Want a tighter cushion? Push the margin to $6\sigma$ and 121 becomes 144. Willing
to tolerate a $0.6\times$ danger line instead of $0.5\times$? It drops. Both knobs
are right there in the equation.
...
The full default also has to clear a hard minimum and any empirical floor from
calibration:

$$\boxed{K = \operatorname{next\_pow2}\left(\max\left(64,\; K_{\text{bias}},\; K_{\text{empirical}}\right)\right)}$$

where $K_{\text{empirical}}$ comes from hardware calibration: bit-position bias,
autocorrelation, afterpulsing, periodic noise, replay tests, and coarse interval
distortion.

The important part is simple:

> 64 is the minimum sane token grain. If the hardware is worse, $K$ rises. If $K$
> rises too far, the hardware is no longer in the floor-fixable regime.

For the measured apparatus, the worst-case imbalance is 55:45, giving
$K_{\text{bias}} \approx 46$ — comfortably under the floor, so the minimum grain
wins and $K = 64$. But that 46 is only half the story: it is the bias of a *raw*
address, and the raw address is not what the CDF ever sees. The spreader comes
first. We will return to what that does to the margin once we have built it.
...
It's worth being explicit about how the spreader and the $K$ floor relate, because they use different exponents — 15 in the spreader, 32 in the $K_{\text{bias}}$ derivation — and that looks like a contradiction until you trace the pipeline order.

They are two independent defenses against the same enemy (PMT imbalance), sitting at two different stages:

The **spreader runs first**, on the raw QRNG word, and can live on the FPGA fabric itself — 32 XOR trees, depth ≈5 LUTs, essentially free. It attacks the *head geometry*: it guarantees that no CDF boundary, however high in the distribution, is decided by a single detector bit. Because every output bit is a parity of at least 15 input bits, a raw per-bit imbalance $\delta$ reaches any token boundary suppressed to $\delta^{15}$. At the measured 5–10% imbalance ($\delta = 0.10$ worst case) that is $10^{-15}$ — gone.

The **$K$ floor is built into the CDF table**, downstream, and attacks the *tail*: it guarantees every token gets at least $K$ slots so nothing in the deep vocabulary is unreachable.

So why does $K_{\text{bias}}$ use the full 32-bit width when the spreader has already crushed the bias to $\delta^{15}$? Because $K$ is sized *as if the spreader did not exist*. The $(1+\delta^2)^{32}$ term is the worst-case bias of a raw, un-spread address — the conservative bound. We deliberately size the floor against that bound rather than the post-spreader reality, so the two mechanisms never have to trust each other. The floor would hold even if the spreader were a no-op; the spreader would hold even if $K$ were 1.

The numbers make this concrete. At the measured worst case, 55:45:

$$K_{\text{bias}} = \left\lceil 121\left((1 + 0.1^2)^{32} - 1\right)\right\rceil \approx 46$$

That sits comfortably below the 64-grain minimum, so the *design* floor wins outright — the bias term never even binds. And that's before the spreader has touched anything. Once it runs, the bias the CDF boundaries actually see is $10^{-15}$, not $\delta$, which means the 46-slot margin we computed was conservative by roughly thirteen orders of magnitude.
